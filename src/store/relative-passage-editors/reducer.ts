...
> = (state, action) => {
	switch (action.type) {
		case 'add': {
			...
			return {
				...state,
				editors: [...state.editors, {passageId: action.passageId, storyId: action.storyId, passageCardPosition: action.passageCardPosition, isNewlyCreated: action.isNewlyCreated}]
			};
		}
